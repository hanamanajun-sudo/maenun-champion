...
const FEATURES = [
  { emoji: '🎯', title: '오늘의 영상', desc: '매일 새로운 AI 의심 영상이 올라와요. 진짜인지 가짜인지 투표해보세요.' },
  { emoji: '🚨', title: '의심 영상 제보', desc: '내가 받은 수상한 영상·사진을 올리면 1,000명 패널이 함께 검증해요.' },
  { emoji: '🏆', title: '점수·뱃지', desc: '맞출수록 점수가 쌓이고, 명예 뱃지를 수집할 수 있어요.' },
];

const NOTIF_ITEMS = [
  { key: 'morning', emoji: '☀️', title: '아침 도전 알림', desc: '매일 오전 7시, 오늘의 영상 도착' },
  { key: 'result',  emoji: '✅', title: '검증 결과 알림', desc: '내가 제보한 영상의 판정이 나오면 알림' },
  { key: 'friend',  emoji: '👥', title: '친구 활동 알림', desc: '내가 공유한 문제를 친구가 풀었을 때' },
  { key: 'urgent',  emoji: '⚠️', title: '긴급 가짜 영상 경보', desc: '바이럴 중인 위험 딥페이크 속보' },
];
  ...
  return (
    <div style={{ minHeight: '100dvh', background: 'var(--bg)', display: 'flex', flexDirection: 'column', maxWidth: 480, margin: '0 auto' }}>
      ...
      <div style={{ padding: '16px 20px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        {step < 3 && (
          <button onClick={handleSkip} style={{ background: 'none', border: 'none', fontSize: 14, color: 'var(--ink-mute)', cursor: 'pointer', padding: '4px 0' }}>
            건너뛰기
          </button>
        )}
      </div>
      ...
      <div style={{ flex: 1, padding: '32px 24px 24px', display: 'flex', flexDirection: 'column' }}>
        ...
        {/* Step 0: 환영 */}
        {step === 0 && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', animation: 'fadeUp 0.3s ease-out both' }}>
        ...
        {/* Step 1: 기능 소개 */}
        {step === 1 && (
          <div style={{ flex: 1, animation: 'fadeUp 0.3s ease-out both' }}>
            <h2 style={{ fontSize: 26, fontWeight: 900, color: 'var(--ink)', letterSpacing: -0.6, marginBottom: 8 }}>이렇게 가려내요</h2>
            <p style={{ fontSize: 14, color: 'var(--ink-mute)', marginBottom: 28 }}>세 가지로 보는 눈을 키워요</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {FEATURES.map(f => (
                <div key={f.title} style={{ background: 'var(--surface)', borderRadius: 16, padding: '18px 20px', display: 'flex', gap: 16, alignItems: 'flex-start', boxShadow: '0 1px 3px rgba(15,30,54,0.06)' }}>
                  <span style={{ fontSize: 36, flexShrink: 0 }}>{f.emoji}</span>
                  <div>
                    <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--ink)', marginBottom: 4 }}>{f.title}</div>
        ...
        {/* Step 2: 별명 */}
        {step === 2 && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', animation: 'fadeUp 0.3s ease-out both' }}>
            <h2 style={{ fontSize: 26, fontWeight: 900, color: 'var(--ink)', letterSpacing: -0.6, marginBottom: 8 }}>별명을 정해요</h2>
            <p style={{ fontSize: 14, color: 'var(--ink-mute)', marginBottom: 28 }}>이름 대신 별명으로 활동해요</p>
            <input
              value={nickname}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 16px', background: 'var(--green-soft)', borderRadius: 12 }}>
              <span>🔒</span>
        ...
        {step === 3 && (
          <div style={{ flex: 1, animation: 'fadeUp 0.3s ease-out both' }}>
            <h2 style={{ fontSize: 26, fontWeight: 900, color: 'var(--ink)', letterSpacing: -0.6, marginBottom: 8 }}>알림 설정</h2>
            <p style={{ fontSize: 14, color: 'var(--ink-mute)', marginBottom: 24 }}>나중에 언제든 바꿀 수 있어요</p>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 16px', background: 'var(--gold-soft)', borderRadius: 12 }}>
              <span>✨</span>
      ...
      <div style={{ padding: '0 24px 32px' }}>
        <button
          ...
          style={{
            ...
            boxShadow: '0 4px 0 #0F254A', transform: 'translateY(0)', transition: 'all 0.1s',
