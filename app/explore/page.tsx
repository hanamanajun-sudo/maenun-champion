...
import { mockMedia, type Media } from '@/lib/mockData';

type Period = 'today' | 'week' | 'month' | 'contested';
type Kind = 'all' | 'video' | 'photo';

function ThumbCard({ media }: { media: Media }) {
    ...
    <Link href={`/explore/${media.id}`} style={{ textDecoration: 'none' }}>
      <div
        style={{
          ...
          boxShadow: '0 1px 4px rgba(15,30,54,0.05)',
          cursor: 'pointer',
        }}
      >
        {/* 썸네일 4:3 */}
        ...
        <div style={{ padding: '10px 10px 12px' }}>
          ...
          <div style={{ fontSize: 11, color: '#7A8499', marginTop: 5, fontWeight: 500 }}>
            {media.totalVotes.toLocaleString()}명 · 💬 코멘트
          </div>
        </div>
      </div>
    </Link>
...
export default function ExplorePage() {
  ...
  const filtered = mockMedia.filter((m) => {
    const periodMatch =
      period === 'contested'
        ? m.contested || Math.abs(Math.round((m.yesCount / m.totalVotes) * 100) - 50) <= 10
        : m.period === period;
    ...
    <div style={{ minHeight: '100dvh', background: 'var(--bg)', paddingBottom: 96 }}>
      ...
      <div style={{ padding: '16px 16px 0' }}>
        ...
        <p style={{ fontSize: 13, color: '#7A8499', fontWeight: 600, margin: '0 0 16px' }}>
          이번 기간 총 {mockMedia.length}건 · {mockMedia.reduce((a, m) => a + m.totalVotes, 0).toLocaleString()}명 참여
        </p>
          ...
          {PERIOD_TABS.map((t) => (
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                whiteSpace: 'nowrap',
                background: period === t.key ? '#1B3A6B' : '#FFFFFF',
                color: period === t.key ? '#FFFFFF' : '#7A8499',
                boxShadow: period === t.key
          ...
          {KIND_FILTERS.map((f) => (
            <button
              ...
              style={{
                ...
                fontWeight: 700,
                cursor: 'pointer',
                border: kind === f.key ? '2px solid #C6953E' : '1.5px solid #DDE3ED',
                background: kind === f.key ? '#FCF3E0' : '#FFFFFF',
                color: kind === f.key ? '#C6953E' : '#7A8499',
                fontFamily: 'inherit',
                transition: 'all 0.15s',
              }}
            >
        ...
        >
          <p style={{ fontSize: 15, fontWeight: 700, color: '#FFFFFF', margin: '0 0 12px', lineHeight: 1.4 }}>
            의심되는 영상이 있으신가요?
          </p>
